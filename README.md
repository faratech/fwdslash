...
On x64 Windows, replace `ARM64` with `x64`. The build uses static C++ runtime
linking and produces no injected DLL.
...
Double-click the tray icon to open the native settings/status window. The
driver-free install is per-user and reversible:
