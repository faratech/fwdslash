...
$stage = Join-Path $packageRoot ("forward-slash-windows-0.0.8-{0}" -f $Architecture.ToLowerInvariant())
$archive = "$stage.zip"
...
# Required, not best-effort: a zip missing any of these ships a settings app
# that cannot start (or a broker without its bootstrapper), with no error
...
foreach ($dependency in 'Microsoft.WindowsAppRuntime.Bootstrap.dll', 'resources.pri', 'fswsettings.pri') {
    ...
    if (-not (Test-Path -LiteralPath $dependencyPath -PathType Leaf)) {
        throw "Required runtime dependency is missing from the build output: $dependencyPath"
    }
